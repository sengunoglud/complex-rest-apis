...
const register = async (req, res) => {
    try {
        ...
        if(user) {
            return res.status(500).json({message: "Bu mail zaten kullanılmış."})
        }

        if(password.length < 6) {
            return res.status(500).json({message: "Parola 6 karakterden büyük olmalı!"})
        }
...
const login = async(req, res) => {
    try {
        ...
        if(!user) {
            return res.status(500).json("Böyle bir kullanıcı bulunamadı!")
        }
